...
│   │   ├── GoogleHomepageSimulation.java      # Homepage test (10-12 users)
│   │   ├── GoogleSearchSimulation.java        # Search workflow test (15 users)
│   │   ├── GoogleStressSimulation.java        # Stress test (30-50 concurrent)
...
### Modify Test Load
Edit simulation file, e.g., `GoogleHomepageSimulation.java`:

```java
// Change from: rampUsers(10).during(Duration.ofSeconds(10))
...
| `GoogleHomepageSimulation.java` | Baseline performance test - homepage access |
| `GoogleSearchSimulation.java` | Realistic workflow test - search scenario |
| `GoogleStressSimulation.java` | Stress/spike test - load resilience |
...
- **Java for Gatling**: https://docs.oracle.com/en/java/
...
3. If you see a Windows `library.jansi.path` error, Maven may be installed under `C:\Program Files`; install Maven in a no-space path like `C:\maven`
4. Check Maven configuration in pom.xml
5. Verify Java/Maven versions match requirements
